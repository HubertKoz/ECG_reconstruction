...
def select_axis_pca(df, columns):
    ...
    # Sprawdzenie korelacji z pierwotną osią Z (często dominującą)
...
def extract_windows(signals, fs, seq_len=250, clean_mask=None, epoch_sec=10, stride=None):
    """
    ...
    ---------
    stride : int lub None
        Krok przesunięcia okna w próbkach.
        None (domyślnie) → stride = seq_len (okna bez nakładania, stare zachowanie).
        Np. stride=64 przy seq_len=250 daje ~3× więcej okien z 75% overlap.
    """
    if stride is None:
        stride = seq_len  # backward-compatible: brak nakładania
    ...
    start = 0
    while start + seq_len <= n_samples_total:
        end = start + seq_len

        if clean_mask is not None:
            epoch_start = start // n_samples_epoch
            epoch_end = (end - 1) // n_samples_epoch
            if (epoch_start >= len(clean_mask) or epoch_end >= len(clean_mask)
                    or not clean_mask[epoch_start] or not clean_mask[epoch_end]):
                start += stride
                continue
